...
def SkinVerts():
    ...
    FaceCreater.add_by_from_pydata(lcMesh, poly_list)
...
def GetFaceEndVerts(inData, inCenterVert):
    ...
    # After gathering and sorting, # print the result
    # print(f"Found {len(outEndVerts)} face-end vertices around vertex {inCenterVert.index}.")
...
def GetFaceEndVertsAroundCenterAndSort(inData):
    ...
    # print("Center Vertex:", inCenterVert.co)
    # print("Number of vertices around:", len(inVertsAround))
    ...
    # Debugging: Check the final number of vertices after processing
    # print(f"Total vertices in VertsAround after processing: {len(inVertsAround)}")
...
def DoAdditionalSearch(inData):
    ...
    for i in range(n):
        ...
        try:
            lcMat = Matrix((
                [lcBase2[0][0] * lcBase2[0][0], lcBase2[0][0] * lcBase2[1][0]],
                [lcBase2[1][0] * lcBase2[0][0], lcBase2[1][0] * lcBase2[1][0]]
            )).inverted()
        except ValueError:
            # Gestisci l'errore se la matrice non è invertibile
            continue
        ...
        for enVert in lcSearchVerts:
            ...
            lcVec = Vector((lcBase2[0][0] * lcVert2[0], lcBase2[1][0] * lcVert2[1])) # 2x1
            lcAns = lcMat @ lcVec
            lcAns = [lcAns[0], lcAns[1], lcZAxis.dot(lcVert2)]
...
def SortVertsAroundZAxis(inData, inVerts):
    ...
    if not inVerts:
        # print("Error: No vertices provided!")
        return []

    # Debugging: # print the number of vertices received
    # print(f"Received {len(inVerts)} vertices to sort.")

    # Transform vertices to defined frame
    lcVerts2 = []
    for enVert in inVerts:
        transformed_vert = inXYTransMat @ (enVert.co - inCenterVert.co)
        lcVerts2.append((transformed_vert, enVert))
    
    # Measure angles and sort vertices by angle around z-axis
    lcAngles = []
    for transformed_vert, enVert in lcVerts2:
        angle = atan2(transformed_vert[1], transformed_vert[0])  # angle in XY plane
        lcAngles.append((angle, enVert))
    
    lcAngles.sort(key=lambda x: x[0])  # Sort by angle
    
    # Pull out sorted vertices
    outVertsAround = [enVert for _, enVert in lcAngles]
    
    # Debugging # print to check the result
    # print("Sorted Vertices:", [enVert.co for enVert in outVertsAround])
...
def GroupVertsByFaceEndVerts(inData):
    ...
    if lcStart is None:
        inData.VertGroups = []  # Imposta VertGroups come lista vuota se non viene trovato alcun vertice
        return False
...
def DiscardVertsHiddenByFaceEnds(inData):
    ...
    for enEndEdge in inEndEdges:
        ...
        try:
            lcInv = Matrix([
                [lcVec2[0].dot(lcVec2[0]), lcVec2[0].dot(lcVec2[1])],
                [lcVec2[1].dot(lcVec2[0]), lcVec2[1].dot(lcVec2[1])]
            ]).inverted()
        ...
        for i, enVert in enumerate(inVertsAround):
            ...
            lcAns = lcInv @ Vector((lcVec2[0].dot(lcVert2), lcVec2[1].dot(lcVert2))) # 2x1
            if lcAns[0] < 0 or lcAns[1] < 0 or lcAns[0] + lcAns[1] < 1: continue
...
def CheckIfAnglesAreTooSmall(inData):
    ...
    for i in lcRange:
        ...
        lcLength = sqrt(lcVec.length ** 2 - (lcVec.dot(inZAxis)) ** 2)
    ...
    def CheckIfItIsInUnableRegion(inVert, inRegion):
        ...
        lcAns = inRegion["Inv"] @ Vector((inRegion["Vec1"].dot(lcVec), inRegion["Vec2"].dot(lcVec))) # 2x1
...
#            # print "!!Near zero 1!!", inVert.index, lcAns
    ...
    for tmpLength, i in lcLengths:
        ...
        try:
            lcInv = Matrix([
                [lcVec1.dot(lcVec1), lcVec1.dot(lcVec2)],
                [lcVec2.dot(lcVec1), lcVec2.dot(lcVec2)]
            ]).inverted()
        except ValueError:
...
def VertsAngle(inSharedVert, inVert1, inVert2):
    ...
    if v1.length == 0 or v2.length == 0:
        return 0  # Restituisce 0 o un altro valore predefinito per vettori di lunghezza zero
...
def VecsAngle2(inVec1, inVec2, inAxis):
    ...
    if v1.length == 0 or v2.length == 0:
        return 0  # Restituisce 0 o un altro valore predefinito per vettori di lunghezza zero
    return v1.angle(v2) * 180 / pi * sign(v1.cross(v2).dot(inAxis)) % 360
...
class FaceCreater:
    ...
    def add_by_vertices_raw(cls, mesh, poly_list):
        ...
        i_begin = len(mesh.polygons)
        if hasattr(poly_list, "__len__"): # if: length is known
            mesh.polygons.add(len(poly_list))
            for i, poly in enumerate(poly_list, i_begin):
                mesh.polygons[i].vertices_raw = cls.__adjust_poly(poly)
...
# Definition of the UI properties
class UIConfig(bpy.types.PropertyGroup):
    
    target_object : StringProperty(name="Target Object", default="Plane", description="Target object")
    skin_only_selected : BoolProperty(name="Skin Only Selected Vertices", default=True, description="Skin only selected vertices in a point cloud")
    ignore_error : BoolProperty(name="Ignore Errors", default=True, description="Ignore any errors and continue the skinning")
    dist_for_skin : FloatProperty(name="Dist for Skin", subtype='DISTANCE', min=0.0001, max=10000, default=1, description="Distance for skin (= s)")
    ratio_for_axis : FloatProperty(name="Ratio for Axis", subtype='FACTOR', min=0.1, max=10, default=2, description="Ratio a*s (Distance for axis = a, Distance for skin = s)")
    ratio_for_grid : FloatProperty(name="Ratio for Grid", subtype='FACTOR', min=0.1, max=10, default=3, description="Ratio g*s (Grid size = g, Distance for skin = s)")
...
class OBJECT_PT_PointCloudSkinner(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if "Errors" in gbLog:
            if gbLog["Errors"] == 0:
                layout_col.label(text="Result: skinned successfully with no errors.")
            elif gbLog["Errors"] > 0:
                layout_col.label(text="Result: occurred %d errors." % gbLog["Errors"])
                layout_col.label(text="The error log has been output to your ")
                layout_col.label(text="clipboard. Please send it to the developer ")
                layout_col.label(text="if you have a trouble. The vertices in ")
                layout_col.label(text="which an error happened are selected.")
...
# Register and unregister functions for Blender 4.2
classes = [UIConfig, OBJECT_PT_PointCloudSkinner, OBJECT_OP_PointCloudSkinner_Skin]

def register():
    for cls in classes:
        bpy.utils.register_class(cls)
    bpy.types.Scene.CONFIG_PointCloudSkinner = PointerProperty(type=UIConfig)
